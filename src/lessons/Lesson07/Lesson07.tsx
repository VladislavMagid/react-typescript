...
function Lesson07() {
  ...
  const getWeather = (
    ...
    decode: (weatherCode: WEATHER_CODES) => string
  ) => {
    console.log(decode(weatherCode));
  };

  getWeather(WEATHER_CODES.BR, decode);
  getWeather(WEATHER_CODES.FC, decode);
  getWeather(WEATHER_CODES.SS, decode);
  getWeather(WEATHER_CODES.FU, decode);

  // 13 Generic
  type ArrayGenerator<ValueType> = ValueType[];
  //   const someStringArray: string[] = ["a", "b", "c"];
  const someStringArray: ArrayGenerator<string> = ["a", "b", "c"];
  const numbers: ArrayGenerator<number> = [1, 2, 3, 4];
